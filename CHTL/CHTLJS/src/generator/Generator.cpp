...
Generator::Generator(std::shared_ptr<CHTL::SaltBridge> bridge) : bridge_(bridge) {}
...
void Generator::visit(const EnhancedSelectorNode& node) {
    if (node.selector == "&") {
        auto parent_selector_any = bridge_->requestData("parent_selector");
        if (parent_selector_any.has_value()) {
            try {
                std::string parent_selector = std::any_cast<std::string>(parent_selector_any);
...
void Generator::visit(const RawJSNode& node) {
