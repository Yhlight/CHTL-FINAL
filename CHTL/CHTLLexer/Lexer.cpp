...
Token Lexer::nextToken() {
    ...
    switch (m_char) {
        case '=':
            if (peekChar() == '=') { readChar(); tok = {TokenType::EQUAL_EQUAL, "==", m_line, startCol}; }
            else { tok = {TokenType::EQUAL, "=", m_line, startCol}; }
            break;
        ...
        case '-':
             if (peekChar() == '>') { readChar(); tok = {TokenType::ARROW, "->", m_line, startCol}; }
             else { tok = {TokenType::MINUS, "-", m_line, startCol}; }
            break;
        case '*':
            if (peekChar() == '*') { readChar(); tok = {TokenType::POWER, "**", m_line, startCol}; }
            else { tok = {TokenType::ASTERISK, "*", m_line, startCol}; }
            break;
        case '/':
            if (peekChar() == '/') { skipSingleLineComment(); return nextToken(); }
            else if (peekChar() == '*') { skipMultiLineComment(); return nextToken(); }
            else { tok = {TokenType::SLASH, "/", m_line, startCol}; }
            break;
        case '<':
            if (peekChar() == '=') { readChar(); tok = {TokenType::LESS_EQUAL, "<=", m_line, startCol}; }
            else { tok = {TokenType::LESS_THAN, "<", m_line, startCol}; }
            break;
        case '>':
             if (peekChar() == '=') { readChar(); tok = {TokenType::GREATER_EQUAL, ">=", m_line, startCol}; }
             else { tok = {TokenType::GREATER_THAN, ">", m_line, startCol}; }
            break;
        case '!':
             if (peekChar() == '=') { readChar(); tok = {TokenType::NOT_EQUAL, "!=", m_line, startCol}; }
            break;
        case '&':
            if (peekChar() == '&') { readChar(); tok = {TokenType::LOGICAL_AND, "&&", m_line, startCol}; }
            else if (peekChar() == '-' && m_position + 2 < m_input.length() && m_input[m_position + 2] == '>') { readChar(); readChar(); tok = {TokenType::EVENT_BIND_OP, "&->", m_line, startCol}; }
            else { tok = {TokenType::AMPERSAND, "&", m_line, startCol}; }
            break;
        case '|':
             if (peekChar() == '|') { readChar(); tok = {TokenType::LOGICAL_OR, "||", m_line, startCol}; }
            break;
        case '"':
        case '\'':
        ...
        case 0:
            ...
            break;
        default:
            if (isalpha(m_char) || m_char == '_') { return readIdentifier(); }
            else if (isdigit(m_char)) { return readNumericLiteral(); }
            break;
    }
