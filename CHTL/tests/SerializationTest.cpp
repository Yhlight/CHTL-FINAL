...
TEST_CASE("Serialization", "[serialization]")
{
    SECTION("ProgramNode")
    {
        ...
        REQUIRE(deserialized_program != nullptr);
        REQUIRE(deserialized_program->GetType() == NodeType::Program);
        auto deserialized_program_node = static_cast<ProgramNode*>(deserialized_program.get());
        REQUIRE(deserialized_program_node->children.size() == 2);
        REQUIRE(deserialized_program_node->children[0]->GetType() == NodeType::Text);
        REQUIRE(static_cast<TextNode*>(deserialized_program_node->children[0].get())->value == "hello");
        REQUIRE(deserialized_program_node->children[1]->GetType() == NodeType::Text);
        REQUIRE(static_cast<TextNode*>(deserialized_program_node->children[1].get())->value == "world");
    }
    ...
    SECTION("ComplexNode")
    {
        ...
        REQUIRE(deserialized_program->GetType() == NodeType::Program);
        auto deserialized_program_node = static_cast<ProgramNode*>(deserialized_program.get());
        REQUIRE(deserialized_program_node->children.size() == 3);

        auto deserialized_element = static_cast<ElementNode*>(deserialized_program_node->children[0].get());
        REQUIRE(deserialized_element->tag_name == "div");
        REQUIRE(deserialized_element->attributes.size() == 1);
        REQUIRE(deserialized_element->attributes[0].name == "id");
        REQUIRE(deserialized_element->attributes[0].value == "main");
        REQUIRE(deserialized_element->children.size() == 1);
        REQUIRE(static_cast<TextNode*>(deserialized_element->children[0].get())->value == "content");

        auto deserialized_style = static_cast<StyleNode*>(deserialized_program_node->children[1].get());
        REQUIRE(deserialized_style->children.size() == 1);
        auto deserialized_rule = static_cast<StyleRuleNode*>(deserialized_style->children[0].get());
        REQUIRE(deserialized_rule->selector == "#main");
        REQUIRE(deserialized_rule->properties.size() == 1);
        auto deserialized_prop = static_cast<StyleProperty*>(deserialized_rule->properties[0].get());
        REQUIRE(deserialized_prop->name == "width");
        auto deserialized_value = static_cast<NumberLiteral*>(deserialized_prop->value.get());
        REQUIRE(deserialized_value->value == 100.0);
        REQUIRE(deserialized_value->unit == "px");

        auto deserialized_script = static_cast<ScriptNode*>(deserialized_program_node->children[2].get());
        REQUIRE(deserialized_script->js_ast != nullptr);
        REQUIRE(deserialized_script->js_ast->children.size() == 1);
        auto deserialized_raw_js = static_cast<CHTLJS::RawJSNode*>(deserialized_script->js_ast->children[0].get());
        REQUIRE(deserialized_raw_js->content == "console.log('hello');");
    }
}
